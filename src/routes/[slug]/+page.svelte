		...
		<svelte:fragment slot="additional-content-top">
			...
			{#if !$settingsStore.hideLanguageHint}
				<HintMessage title="Did you know?" variantClass="variant-ghost-surface">
					<p>
						The AI understands various languages. You can just ask your questions in German if you
						like.
					</p>
					<svelte:fragment slot="actions">
			...
			>
				...
				{#if !hasContext && !hasStopSequence}
					<p>
						Give the Ai <strong>context</strong> so that the responses are more accurate and less general.
						<strong>For test questions</strong> This is extremely useful because answers need to be as accurate as possible.
					</p>
				{/if}
